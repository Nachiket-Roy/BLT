...
+(function ($) {
  ...
  const clickHandler = function (e) {
    let href;
    const $this = $(this);
    const $target = $($(sanitizeSelector($this.attr('data-target'))) || ((href = sanitizeSelector($this.attr('href'))) && href.replace(/.*(?=#[^\s]+$)/, '')));
...
+(function ($) {
  ...
  Collapse.prototype.getParent = function () {
    const sanitizedParent = sanitizeSelector(this.options.parent);
  ...
  function getTargetFromTrigger ($trigger) {
    let href;
    const target = sanitizeSelector($trigger.attr('data-target')) ||
...
+(function ($) {
  ...
  function getParent ($this) {
    let selector = sanitizeSelector($this.attr('data-target'));
...
+(function ($) {
  ...
  $(document).on('click.bs.modal.data-api', '[data-toggle="modal"]', function (e) {
    ...
    const $target = $((sanitizeSelector($this.attr('data-target'))) || (href && sanitizeSelector(href.replace(/.*(?=#[^\s]+$)/, ''))));
...
+(function ($) {
  ...
  ScrollSpy.prototype.activate = function (target) {
    ...
    let active = $(escapePotentialXSS(selector))
      .parents('li')
      .addClass('active');
