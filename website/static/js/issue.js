...
window.twttr = (function (d, s, id) {
  let js; const fjs = d.getElementsByTagName(s)[0]; const t = window.twttr || {};
...
$(function () {
  ...
  $(document).on('click', '.edit_form button[type="submit"]', function (e) {
    e.preventDefault();
    const issue_id = $('#issue_pk').val();
    const comment = $(this).prev().find('textarea').val();
    if (comment == '') return;
    $.ajax({
      type: 'GET',
