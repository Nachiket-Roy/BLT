...
(function (root, factory) {
  ...
  function _introForElement (targetElm) {
    ...
    if (this._options.steps) {
      ...
      for (var i = 0, stepsLength = this._options.steps.length; i < stepsLength; i++) {
        const currentItem = _cloneObject(this._options.steps[i]);
        // set the step
    ...
    if (_addOverlayLayer.call(self, targetElm)) {
      ...
      const skipButton = targetElm.querySelector('.introjs-skipbutton');
      const nextStepButton = targetElm.querySelector('.introjs-nextbutton');
  ...
  function _nextStep () {
    ...
    if (typeof (this._currentStepNumber) !== 'undefined') {
      for (let i = 0, len = this._introItems.length; i < len; i++) {
  ...
  function _determineAutoPosition (targetElement, tooltipLayer, desiredTooltipPosition) {
    ...
    const tooltipHeight = _getOffset(tooltipLayer).height + 10;
    const tooltipWidth = _getOffset(tooltipLayer).width + 20;
    const targetOffset = _getOffset(targetElement);
  ...
  function _setHelperLayerPosition (helperLayer) {
    if (helperLayer) {
      ...
      const elementPosition = _getOffset(currentElement.element);
      let widthHeightPadding = 10;
  ...
  function _showElement (targetElement) {
    ...
    const elementPosition = _getOffset(targetElement.element);
    ...
    if (oldHelperLayer != null) {
      ...
      var skipTooltipButton = oldReferenceLayer.querySelector('.introjs-skipbutton');
      var prevTooltipButton = oldReferenceLayer.querySelector('.introjs-prevbutton');
      var nextTooltipButton = oldReferenceLayer.querySelector('.introjs-nextbutton');
    ...
    if (!_elementInViewport(targetElement.element) && this._options.scrollToElement === true) {
      ...
      const winHeight = _getWinSize().height;
      const top = rect.bottom - (rect.bottom - rect.top);
      const bottom = rect.bottom - winHeight;
  ...
  function _setShowElement (targetElement) {
    ...
    const currentElementPosition = _getPropValue(targetElement.element, 'position');
    ...
    while (parentElm != null) {
      ...
      const zIndex = _getPropValue(parentElm, 'z-index');
      const opacity = parseFloat(_getPropValue(parentElm, 'opacity'));
      const transform = _getPropValue(parentElm, 'transform') || _getPropValue(parentElm, '-webkit-transform') || _getPropValue(parentElm, '-moz-transform') || _getPropValue(parentElm, '-ms-transform') || _getPropValue(parentElm, '-o-transform');
  ...
  function _addOverlayLayer (targetElm) {
    ...
    if (!targetElm.tagName || targetElm.tagName.toLowerCase() === 'body') {
      ...
      const elementPosition = _getOffset(targetElm);
      if (elementPosition) {
  ...
  function _removeHintTooltip () {
    const tooltip = this._targetElement.querySelector('.introjs-hintReference');
  ...
  function _populateHints (targetElm) {
    ...
    if (this._options.hints) {
      for (var i = 0, l = this._options.hints.length; i < l; i++) {
        const currentItem = _cloneObject(this._options.hints[i]);
  ...
  function _reAlignHints () {
    for (let i = 0, l = this._introItems.length; i < l; i++) {
      const item = this._introItems[i];
  ...
  function _hideHint (stepId) {
    ...
    const hint = this._targetElement.querySelector('.introjs-hint[data-step="' + stepId + '"]');
  ...
  /**
     ...
     */
  function _hideHints () {
    const hints = this._targetElement.querySelectorAll('.introjs-hint');
  ...
  function _addHints () {
    ...
    const oldHintsWrapper = document.querySelector('.introjs-hints');
  ...
  function _alignHintPosition (position, hint, element) {
    ...
    const offset = _getOffset.call(this, element);
    const iconWidth = 20;
    const iconHeight = 20;
  ...
  function _hintClick (hintElement, item, stepId) {
    ...
    const removedStep = _removeHintTooltip.call(this);
  ...
  const introJs = function (targetElm) {
