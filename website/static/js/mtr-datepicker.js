...
function MtrDatepicker (inputConfig) {
  ...
  const defaultChangeEventsCategories = {
    all: [],
    time: [],
    date: [],
  ...
  /**
	 ...
	 */
  const init = function (inputConfig) {
  ...
  var setConfig = function (input) {
    ...
    const localTimezoneOffsetTimestamp = values.date.getTime() + (values.date.getTimezoneOffset() * 60 * 1000);
    const timezoneOffsetTimestamp = localTimezoneOffsetTimestamp + (config.utcTimezone.offset * 60 * 60 * 1000);
  ...
  var setDatesRange = function (month, year) {
    ...
    const datesRange = createRangeForDate(month, year);
    config.dates = {
      min: datesRange.min,
      max: datesRange.max,
  ...
  var createMarkup = function () {
    ...
    const hoursElement = createSliderInput({
      name: 'hours',
      values: config.defaultValues.hours,
      value: getHours()
    });

    const minutesElement = createSliderInput({
      name: 'minutes',
    ...
    const rowClearfixTime = document.createElement('div');
    ...
    const monthElement = createSliderInput({
      name: 'months',
    ...
    const dateElement = createSliderInput({
      name: 'dates',
    ...
    const yearElement = createSliderInput({
      name: 'years',
    ...
    const rowClearfixDate = document.createElement('div');
  ...
  var createSliderInput = function (elementConfig) {
    ...
    const divArrowUp = createUpArrow();
    const divArrowDown = createDownArrow();
    ...
    const divValues = createValues(inputValue);
    ...
    function createUpArrow () {
      ...
      divArrowUp.addEventListener('click', function () {
        // Prevent blur event
        const input = qSelect(inputValue, '.mtr-input');
        ...
        let indexInArray = config.defaultValues[name].indexOf(currentValue);
        indexInArray++;
    ...
    function createInputValue () {
      ...
      inputValue.addEventListener('blur', function (e) {
        ...
        function blurEvent () {
          ...
          let oldValue = inputValue.getAttribute('data-old-value');
          ...
          let target = elementConfig.name.substring(0, elementConfig.name.length - 1);
      ...
      inputValue.addEventListener('wheel ', function (e) {
        ...
        const wheelData = e.wheelDeltaY ? e.wheelDeltaY : (e.deltaY * -1);

        const oldValue = parseInt(inputValue.value);
        let newValue;

        let configMin = config[elementConfig.name].min;
        let configMax = config[elementConfig.name].max;
        const configStep = config[elementConfig.name].step;
    ...
    function createValues (inputValue) {
      const divValues = createElementValues(elementConfig);
      ...
      divValues.addEventListener('touchmove', function (e) {
        handleTouchMove(e, function (direction) {
          const parent = divValues.parentElement.parentElement;
          let arrow;
  ...
  var createRadioInput = function (elementConfig) {
    ...
    const radioAm = createInputValue('ampm', 1, 'AM');
    const radioPm = createInputValue('ampm', 0, 'PM');
    ...
    function createInputValue (radioName, radioValue, labelValue) {
      ...
      const elementId = config.targetElement + '-radio-' + radioName + '-' + labelValue;

      const innerHtmlSpanValue = document.createElement('span');
      innerHtmlSpanValue.className = 'value';
      innerHtmlSpanValue.appendChild(document.createTextNode(labelValue));

      const innerHtmlSpanRadio = document.createElement('span');
      ...
      input.addEventListener('change', function (e) {
        const result = setAmPm(radioValue);
  ...
  var createElementValues = function (elementConfig) {
    ...
    elementConfig.values.forEach(function (value) {
      const innerHTML = elementConfig.name === 'months' ? value + 1 : value;

      const divValueHolder = document.createElement('div');
      ...
      if (elementConfig.valuesNames) {
        const divValueName = document.createElement('div');
    ...
    // Attach listeners
    const inputClickEventListener = function () {
      // Show the input field for manual setup
      const parent = divValues.parentElement;
      const inputValue = qSelect(parent, '.mtr-input');
    ...
    divValues.addEventListener('wheel', function (e) {
      ...
      // If the user is using the mouse wheel the values should be changed
      const target = e.target;
      const parent = target.parentElement.parentElement.parentElement.parentElement; // value -> values -> content -> input slider
      const values = qSelect(parent, '.mtr-values');
      const input = qSelect(parent, '.mtr-input');
      const wheelData = e.wheelDeltaY ? e.wheelDeltaY : (e.deltaY * -1); // Firefox doesn't support wheelDataY, so we are using deltaY and cghanging the sign of the value
  ...
  const rebuildElementValues = function (reference, data) {
    const element = byId(reference);
    const elementContent = qSelect(element, '.mtr-content');
    const elementContentValues = qSelect(elementContent, '.mtr-values');

    elementContentValues.parentNode.removeChild(elementContentValues);
    const elementContentNewValues = createElementValues({
      name: data.name,
      values: data.values,
  ...
  /**
	 ...
	 * @param  {NUmber} newYar
	 */
  const updateDate = function (newMonth, newYear) {
    ...
    const maxDay = config.defaultValues.dates[config.defaultValues.dates.length - 1];
  ...
  var validateChange = function (target, newValue, oldValue) {
    ...
    const datePicker = new Date(values.date.getTime());
    ...
    case 'ampm':
      ...
      var currentAmPm = !!((currentHours >= 0 && currentHours <= 11));
  ...
  var setHours = function (input, preventAnimation) {
    let oldValue = values.date.getHours();
    let isChangeValid = validateChange('hour', input, oldValue);
    const isAm = getIsAm();
  ...
  var getHours = function () {
    const currentHours = values.date.getHours();
  ...
  var setMinutes = function (input, preventAnimation) {
    const oldValue = values.date.getMinutes();
    const isChangeValid = validateChange('minute', input, oldValue);
    ...
    // TODO: validate
    const defaultValues = config.defaultValues.minutes;
  ...
  var setAmPm = function (setAmPm) {
    ...
    let currentHours = values.date.getHours();
    const currentHoursCalculates = getHours();

    const currentIsAm = getIsAm();
  ...
  var setRadioFormValue = function (reference, setAmPm) {
    ...
    const divRadioInput = byId(reference);
    const formRadio = qSelect(divRadioInput, 'form');

    formRadio.ampm.value = setAmPm ? '1' : '0';
    const labelAmPm = setAmPm ? 'AM' : 'PM';

    const radioAm = qSelect(formRadio, 'input.mtr-input[type="radio"][value="1"]');
    const radioPm = qSelect(formRadio, 'input.mtr-input[type="radio"][value="0"]');

    const label = qSelect(formRadio, 'label[for="' + config.targetElement + '-radio-ampm-' + labelAmPm + '"]');
    const checkbox = qSelect(label, 'checkbox');
  ...
  var setDate = function (newDate, preventAnimation) {
    const oldValue = values.date.getDate();
    const isChangeValid = validateChange('day', newDate, oldValue);
  ...
  var setMonth = function (newMonth, preventAnimation) {
    const oldValue = values.date.getMonth();
    const isChangeValid = validateChange('month', newMonth, oldValue);
  ...
  var setYear = function (newYear, preventAnimation) {
    const oldValue = values.date.getFullYear();
    const isChangeValid = validateChange('year', newYear, oldValue);
  ...
  const getFullTime = function () {
    return getHours() + ':' + getMinutes() + ' ' + (getIsAm() ? 'AM' : 'PM');
  };

  var setTimestamp = function (input) {
    const roundedTimestamp = roundUpTimestamp(input);
    ...
    const currentAmPm = !!((currentHours >= 0 && currentHours < 12));
    const currentDate = getDate();
  ...
  var updateInputSlider = function (reference, newValue, preventAnimation) {
    ...
    const divValues = qSelect(element, '.mtr-content');
    const divValue = qSelect(element, '.mtr-values .mtr-default-value[data-value="' + newValue + '"]');
    const divArrow = qSelect(element, '.mtr-arrow.up');
    const inputValue = qSelect(element, '.mtr-input');
  ...
  var showInputSliderError = function (reference) {
    const element = byId(reference);
    const divContent = qSelect(element, '.mtr-content');
  ...
  var showInputRadioError = function (reference, value) {
    ...
    const element = byId(reference);
    const divContent = qSelect(element, '.mtr-input[value="' + value + '"]');
    addClass(divContent, 'mtr-error');
  ...
  function createRange (settings) {
    const from = settings.min;
    const to = settings.max;
  ...
  var roundUpTimestamp = function (timestamp) {
    const border = config.minutes.step * 60 * 1000;
    let delta = 0;
  ...
  var toTimeString = function () {
    if (plugins.timezones) {
      let toReturn = '';
      const timeString = values.date.toTimeString().split(' ');
  ...
  const format = function (input) {
    ...
    const currentMinutes = getMinutes();
    const currentAmPm = getIsAm();

    const currentDate = getDate();
    const currentMonth = getMonth() + 1;
    const currentYear = getYear();
    const currentTimezone = config.utcTimezone.offset;
    ...
    function specialReplace (input, selector, value) {
      ...
      const regex = new RegExp(selector + '(?!' + specialDelimiter + ')', 'g');
  ...
  const onChange = function (target, callback) {
    events.onChange[target].push(callback);
  };

  const beforeChange = function (target, callback) {
    events.beforeChange[target].push(callback);
  };

  const afterChange = function (target, callback) {
